/*  Student information for assignment:
 *
 *  On my honor, <Manuel Ponce>, this programming assignment is my own work
 *  and I have not provided this code to any other student.
 *
 *  Name: Manuel Ponce	
 *  email address: maniponce22@gmail.com
 *  UTEID: mip445
 *  Section 5 digit ID: 51399
 *  Grader name: Lucas
 *  Number of slip days used on this assignment: 0
 */

// Experiment results. CS314 students, place your experiment
// results here:
// 		Adding to the end was A LOT faster in the LinkedList. O(1) for this becuase we went over in class.
//		Adding to the front was A LOT faster in the LinkedList. O(1) b/c no shifting
//		Removing from the front was A LOT faster for LinkedList. O(1) b/c no shifting
//		Getting a random element was A LOT faster in the ArrayList. O(N) becuase have to traverse the list
//		Getting all using the iterator was almost the same for both, Linked list was a little faster. O(1) iterator is fast.
//		Getting all using the get method was A LOT faster in ArrayList. O(N) b/c have to go through list



import java.util.Iterator;
import java.util.ArrayList;
import java.util.Random;
import java.util.Arrays;

public class LinkedListTester {
    public static void main(String[] args) {



        //CS314 students. Add your tests here:
    	test1();
    	test2();
    	test3();
    	
        // CS314 Students:
        // uncomment the following line to run tests comparing
        // your LinkedList class to the java ArrayList class.
        comparison();
    }
    
    public static void test1() {
    	//***SET TEST 1***//   	
        LinkedList<Integer> list1 = new LinkedList<Integer>();
        
	        //Test: addFirst
        		System.out.println("1ST SET OF TESTS");
		        list1.addFirst(1); list1.addFirst(2); list1.addFirst(3); list1.addFirst(4); 
		        if (list1.toString().equals("[4, 3, 2, 1]"))
		        		System.out.println("Passed addFirst Test");
		        else
		        	System.out.println("Failed addFirst Test");
	        	
	        //Test: addLast
		        list1.addLast(4);
		        if (list1.toString().equals("[4, 3, 2, 1, 4]"))
		        		System.out.println("Passed addLast Test");
		        else
		        	System.out.println("Failed addLast Test");
	        //Test: removeFirst
		        int tempRemFirst = list1.removeFirst();
		        //[3, 2, 1, 4]
		        if (tempRemFirst == 4)
		        		System.out.println("Passed removeFirst Test");
		        else
		        	System.out.println("Failed removeFirst Test");
	        //Test: removeLast
		        int tempRemLast = list1.removeLast();
		        //[3, 2, 1]
		        if (tempRemLast == 4)
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: add
		        list1.makeEmpty();
		        list1.add(1); list1.add(2); list1.add(1); list1.add(1);
		        //[1, 2, 1, 1]
		        if (list1.toString().contentEquals("[1, 2, 1, 1]"))
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: insert
		        list1.insert(1, 3);
		        //[1, ,3, 2, 1, 1]
		        if (list1.toString().contentEquals("[1, 3, 2, 1, 1]"))
		        		System.out.println("Passed insert Test");
		        else
		        	System.out.println("Failed insert Test");
	        //Test: set
		        list1.set(4, 3);
		        //[1, ,3, 2, 1, 3]
		        if (list1.toString().contentEquals("[1, 3, 2, 1, 3]"))
		        		System.out.println("Passed set Test");
		        else
		        	System.out.println("Failed set Test");
	        //Test: get
		        //[1, ,3, 2, 1, 3]
		        if (list1.get(4) == 3)
		        		System.out.println("Passed get Test");
		        else
		        	System.out.println("Failed get Test");
	        //Test: remove(int)
		        list1.remove(4);
		        //[1, ,3, 2, 1]
		        if (list1.toString().contentEquals("[1, 3, 2, 1]"))
		        		System.out.println("Passed remove(pos) Test");
		        else
		        	System.out.println("Failed remove(pos) Test");
	        //Test: remove(Obj)
		         boolean removed = list1.remove((Integer)3);
		        //[1, 2, 1]
		        if (removed)
		        		System.out.println("Passed remove(item) Test");
		        else
		        	System.out.println("Failed remove(item) Test");
	        //Test: getSubList
		        list1.makeEmpty(); list1.add(3); list1.add(3); list1.add(2); list1.add(2);
		        //[3, 3, 2, 2]
		        IList<Integer> tempL = list1.getSubList(2, 3);
		        if (tempL.toString().contentEquals("[2]"))
		        		System.out.println("Passed subList Test");
		        else
		        	System.out.println("Failed subList Test");
	        //Test: size
		        //[3, 3, 2, 2]
		        if (list1.size() == 4)
		        		System.out.println("Passed size Test");
		        else
		        	System.out.println("Failed size Test");
		    //Test: indexOf(item)
		        //[3, 3, 2, 2]
		        if (list1.indexOf(2) == 2)
		        		System.out.println("Passed indexOf(obj) Test"); 
		        else
		        	System.out.println("Failed indexOf(obj) Test");
	        //Test: index(pos, item)
		        //[3, 3, 2, 2, 5]
		        list1.add(5);
		        if (list1.indexOf(2, 2) == 2)
		        		System.out.println("Passed indexOf(pos, obj) Test");
		        else
		        	System.out.println("Failed indexOf(pos, obj) Test");
	        //Test: makeEmpty
		        list1.makeEmpty();
		        if (list1.toString().equals("[]"))
		        		System.out.println("Passed makeEmpty Test"); 
		        else
		        	System.out.println("Failed makeEmpty Test");
	        //Test: iterator
		        Iterator<Integer> LLIter = list1.iterator();
		        int iterCount = 0;
		        while (LLIter.hasNext()) {
		        	LLIter.next();
		        	iterCount++;
		        }
		        if (iterCount == list1.size())
	        		System.out.println("Passed Iterator hasNext Test"); 
		        else
		        	System.out.println("Failed Iterator Test");
	        //Test: removeRange
		        list1.makeEmpty();
		        list1.add(1);  list1.add(2);  list1.add(3);  list1.add(4); list1.add(5);
		        list1.removeRange(0, 3);
		        if (list1.toString().equals("[4, 5]"))
		        		System.out.println("Passed removeRange Test"); 
		        else
		        	System.out.println("Failed removeRange Test");
	        //Test: toString
		        if (list1.toString().equals("[4, 5]"))
	        		System.out.println("Passed toString Test"); 
		        else
		        	System.out.println("Failed toString Test");		        
	        //Test: equals
		        LinkedList<Integer> other1 = list1;
		        if (list1.equals(other1))
	        		System.out.println("Passed equals Test"); 
		        else
		        	System.out.println("Failed equals Test");
    	}

    public static void test2() {
    	//***SET TEST 2***//   	
        LinkedList<Integer> list2 = new LinkedList<Integer>();
        
	        //Test: addFirst
        		System.out.println("\n SECOND SET OF TESTS");
        		list2.add(1);
		        if (list2.toString().equals("[1]"))
		        		System.out.println("Passed addFirst Test");
		        else
		        	System.out.println("Failed addFirst Test");
	        	
	        //Test: addLast
		        list2.makeEmpty();
		        list2.addLast(1);
		        if (list2.toString().equals("[1]"))
		        		System.out.println("Passed addLast Test");
		        else
		        	System.out.println("Failed addLast Test");
	        //Test: removeFirst
		        int tempRemFirst = list2.removeFirst();
		        //[]
		        if (tempRemFirst == 1)
		        		System.out.println("Passed removeFirst Test");
		        else
		        	System.out.println("Failed removeFirst Test");
	        //Test: removeLast
		        list2.add(1);
		        int tempRemLast = list2.removeLast();
		        //[]
		        if (tempRemLast == 1)
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: add
		        list2.makeEmpty();
		        list2.add(1);
		        //[1]
		        if (list2.toString().contentEquals("[1]"))
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: insert
		        list2.makeEmpty();
		        list2.insert(0, 3);
		        //[1, ,3, 2, 1, 1]
		        if (list2.toString().contentEquals("[3]"))
		        		System.out.println("Passed insert Test");
		        else
		        	System.out.println("Failed insert Test");
	        //Test: set
		        list2.makeEmpty();
		        list2.add(9);
		        list2.set(0, 3);
		        if (list2.toString().contentEquals("[3]"))
		        		System.out.println("Passed set Test");
		        else
		        	System.out.println("Failed set Test");
	        //Test: get
		        //[1, ,3, 2, 1, 3]
		        if (list2.get(0) == 3)
		        		System.out.println("Passed get Test");
		        else
		        	System.out.println("Failed get Test");
	        //Test: remove(int)
		        list2.remove(0);
		        //[1, ,3, 2, 1]
		        if (list2.toString().contentEquals("[]"))
		        		System.out.println("Passed remove(pos) Test");
		        else
		        	System.out.println("Failed remove(pos) Test");
	        //Test: remove(Obj)
		        list2.add(3);
		         boolean removed = list2.remove((Integer)3);
		        //[1, 2, 1]
		        if (removed)
		        		System.out.println("Passed remove(item) Test");
		        else
		        	System.out.println("Failed remove(item) Test");
	        //Test: getSubList
		        list2.makeEmpty(); list2.add(3); list2.add(3);
		        IList<Integer> tempL = list2.getSubList(0, 1);
		        if (tempL.toString().contentEquals("[3]"))
		        		System.out.println("Passed subList Test");
		        else
		        	System.out.println("Failed subList Test");
	        //Test: size
		        //[3, 3, 2, 2]
		        if (list2.size() == 2)
		        		System.out.println("Passed size Test");
		        else
		        	System.out.println("Failed size Test");
		    //Test: indexOf(item)
		        //[3, 3, 2, 2]
		        if (list2.indexOf(3) == 0)
		        		System.out.println("Passed indexOf(obj) Test"); 
		        else
		        	System.out.println("Failed indexOf(obj) Test");
	        //Test: index(pos, item)
		        //[3, 3, 2, 2, 5]
		        list2.makeEmpty();
		        list2.add(5);
		        if (list2.indexOf(5, 0) == 0)
		        		System.out.println("Passed indexOf(pos, obj) Test");
		        else
		        	System.out.println("Failed indexOf(pos, obj) Test");
	        //Test: makeEmpty
		        list2.makeEmpty();
		        if (list2.toString().equals("[]"))
		        		System.out.println("Passed makeEmpty Test"); 
		        else
		        	System.out.println("Failed makeEmpty Test");
	        //Test: iterator
		        Iterator<Integer> LLIter = list2.iterator();
		        int iterCount = 0;
		        while (LLIter.hasNext()) {
		        	LLIter.next();
		        	iterCount++;
		        }
		        if (iterCount == list2.size())
	        		System.out.println("Passed Iterator hasNext Test"); 
		        else
		        	System.out.println("Failed Iterator Test");
	        //Test: removeRange
		        list2.makeEmpty();
		        list2.add(1);  list2.add(2);
		        list2.removeRange(0, 1);
		        if (list2.toString().equals("[2]"))
		        		System.out.println("Passed removeRange Test"); 
		        else
		        	System.out.println("Failed removeRange Test");
	        //Test: toString
		        if (list2.toString().equals("[2]"))
	        		System.out.println("Passed toString Test"); 
		        else
		        	System.out.println("Failed toString Test");		        
	        //Test: equals
		        LinkedList<Integer> other1 = list2;
		        if (list2.equals(other1))
	        		System.out.println("Passed equals Test"); 
		        else
		        	System.out.println("Failed equals Test");
    	}
    public static void test3() {
    	//***3RD SET TEST ***//   	
        LinkedList<Integer> list3 = new LinkedList<Integer>();
        
	        //Test: addFirst
        		System.out.println("\n 3RD SET OF TESTS");
        		list3.add(1);
		        if (list3.toString().equals("[1]"))
		        		System.out.println("Passed addFirst Test");
		        else
		        	System.out.println("Failed addFirst Test");
	        	
	        //Test: addLast
		        list3.makeEmpty();
		        list3.addLast(5);
		        if (list3.toString().equals("[5]"))
		        		System.out.println("Passed addLast Test");
		        else
		        	System.out.println("Failed addLast Test");
	        //Test: removeFirst
		        int tempRemFirst = list3.removeFirst();
		        //[]
		        if (tempRemFirst == 5)
		        		System.out.println("Passed removeFirst Test");
		        else
		        	System.out.println("Failed removeFirst Test");
	        //Test: removeLast
		        list3.add(1);
		        int tempRemLast = list3.removeLast();
		        //[]
		        if (tempRemLast == 1)
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: add
		        list3.makeEmpty();
		        list3.add(1);
		        //[1]
		        if (list3.toString().contentEquals("[1]"))
		        		System.out.println("Passed removeLast Test");
		        else
		        	System.out.println("Failed removeLast Test");
	        //Test: insert
		        list3.makeEmpty();
		        list3.insert(0, 3);
		        //[1, ,3, 2, 1, 1]
		        if (list3.toString().contentEquals("[3]"))
		        		System.out.println("Passed insert Test");
		        else
		        	System.out.println("Failed insert Test");
	        //Test: set
		        list3.makeEmpty();
		        list3.add(9);
		        list3.set(0, 3);
		        if (list3.toString().contentEquals("[3]"))
		        		System.out.println("Passed set Test");
		        else
		        	System.out.println("Failed set Test");
	        //Test: get
		        //[1, ,3, 2, 1, 3]
		        if (list3.get(0) == 3)
		        		System.out.println("Passed get Test");
		        else
		        	System.out.println("Failed get Test");
	        //Test: remove(int)
		        list3.remove(0);
		        //[1, ,3, 2, 1]
		        if (list3.toString().contentEquals("[]"))
		        		System.out.println("Passed remove(pos) Test");
		        else
		        	System.out.println("Failed remove(pos) Test");
	        //Test: remove(Obj)
		        list3.add(3);
		         boolean removed = list3.remove((Integer)3);
		        //[1, 2, 1]
		        if (removed)
		        		System.out.println("Passed remove(item) Test");
		        else
		        	System.out.println("Failed remove(item) Test");
	        //Test: getSubList
		        list3.makeEmpty(); list3.add(3); list3.add(3);
		        IList<Integer> tempL = list3.getSubList(0, 1);
		        if (tempL.toString().contentEquals("[3]"))
		        		System.out.println("Passed subList Test");
		        else
		        	System.out.println("Failed subList Test");
	        //Test: size
		        //[3, 3, 2, 2]
		        if (list3.size() == 2)
		        		System.out.println("Passed size Test");
		        else
		        	System.out.println("Failed size Test");
		    //Test: indexOf(item)
		        //[3, 3, 2, 2]
		        if (list3.indexOf(3) == 0)
		        		System.out.println("Passed indexOf(obj) Test"); 
		        else
		        	System.out.println("Failed indexOf(obj) Test");
	        //Test: index(pos, item)
		        //[3, 3, 2, 2, 5]
		        list3.makeEmpty();
		        list3.add(5);
		        if (list3.indexOf(8, 0) == -1)
		        		System.out.println("Passed indexOf(pos, obj) Test");
		        else
		        	System.out.println("Failed indexOf(pos, obj) Test");
	        //Test: makeEmpty
		        list3.makeEmpty();
		        if (list3.toString().equals("[]"))
		        		System.out.println("Passed makeEmpty Test"); 
		        else
		        	System.out.println("Failed makeEmpty Test");
	        //Test: iterator
		        Iterator<Integer> LLIter = list3.iterator();
		        int iterCount = 0;
		        while (LLIter.hasNext()) {
		        	LLIter.next();
		        	iterCount++;
		        }
		        if (iterCount == list3.size())
	        		System.out.println("Passed Iterator hasNext Test"); 
		        else
		        	System.out.println("Failed Iterator Test");
	        //Test: removeRange
		        list3.makeEmpty();
		        list3.add(1);  list3.add(2);
		        list3.removeRange(0, 1);
		        if (list3.toString().equals("[2]"))
		        		System.out.println("Passed removeRange Test"); 
		        else
		        	System.out.println("Failed removeRange Test");
	        //Test: toString
		        if (list3.toString().equals("[2]"))
	        		System.out.println("Passed toString Test"); 
		        else
		        	System.out.println("Failed toString Test");		        
	        //Test: equals
		        LinkedList<Integer> other1 = list3;
		        if (list3.equals(other1))
	        		System.out.println("Passed equals Test"); 
		        else
		        	System.out.println("Failed equals Test");
    	}

    private static Object[] toArray(LinkedList<String> list) {
        Object[] result = new Object[list.size()];
        Iterator<String> it = list.iterator();
        int index = 0;
        while( it.hasNext() ){
            result[index] = it.next();
            index++;
        }
        return result;
    }

    //pre: none
    private static boolean arraysSame(Object[] one, Object[] two)  {
        boolean same;
        if( one == null || two == null ) {
            same = (one == two);
        }
        else {
            //neither one or two are null
            assert one != null && two != null;
            same = one.length == two.length;
            if( same ) {
                int index = 0;
                while( index < one.length && same ) {
                    same = ( one[index].equals(two[index]) );
                    index++;
                }
            }
        }
        return same;
    }
   
    
    public static final int NUM_DOUBLINGS_OF_N = 5;
    private static final int NUM_REPEATS_OF_TEST = 100;

    // A method to be run to compare the LinkedList you are completing and the Java ArrayList class
    public static void comparison(){
        Stopwatch s = new Stopwatch();
        
        int initialN = 30000;
        addEndArrayList(s, initialN, NUM_DOUBLINGS_OF_N);
        addEndLinkedList(s, initialN, NUM_DOUBLINGS_OF_N);

        initialN = 2000;
        addFrontArrayList(s, initialN, NUM_DOUBLINGS_OF_N);
        initialN = 10000;
        addFrontLinkedList(s, initialN, NUM_DOUBLINGS_OF_N);

        initialN = 2000;
        removeFrontArrayList(s, initialN, NUM_DOUBLINGS_OF_N);
        initialN = 5000;
        removeFrontLinkedList(s, initialN, NUM_DOUBLINGS_OF_N);

        initialN = 10000;
        getRandomArrayList(s, initialN, NUM_DOUBLINGS_OF_N);
        initialN = 1000;
        getRandomLinkedList(s, initialN, NUM_DOUBLINGS_OF_N);

        initialN = 50000;
        getAllArrayListUsingIterator(s, initialN, NUM_DOUBLINGS_OF_N);
        getAllLinkedListUsingIterator(s, initialN, NUM_DOUBLINGS_OF_N);

        initialN = 100000;
        getAllArrayListUsingGetMethod(s, initialN, NUM_DOUBLINGS_OF_N);
        initialN = 1000;
        getAllLinkedListUsingGetMethod(s, initialN, NUM_DOUBLINGS_OF_N);

    }

    // These pairs of method illustarte a failure to use polymorphism
    // If the students had implemented the Java list interface there
    // could be a single method. Also we could use function objects to
    // reduce the awful repitition of code.
    public static void addEndArrayList(Stopwatch s, int initialN, int numTests){

        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                ArrayList<Integer> javaList = new ArrayList<Integer>();
                s.start();
                for(int j = 0; j < n; j++)
                    javaList.add(j);
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Adding at end: ArrayList", totalTimes, initialN);
    }

    private static void showResults(String title, double[] times, int initialN) {
        System.out.println();
        System.out.println("Num Repeats: " + NUM_REPEATS_OF_TEST);
        System.out.println(title);
        for(double time : times) {
            System.out.print("N = " + initialN + ", total time: ");
            System.out.printf("%7.4f\n", time);
            
            initialN *= 2;
        }
        System.out.println();
    }

    public static void addEndLinkedList(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                LinkedList<Integer> studentList = new LinkedList<Integer>();
                s.start();
                for(int j = 0; j < n; j++)
                    studentList.add(j);
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Adding at end: LinkedList", totalTimes, initialN);
    }

    public static void addFrontArrayList(Stopwatch s, int initialN, int numTests){

        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                ArrayList<Integer> javaList = new ArrayList<Integer>();
                s.start();
                for(int j = 0; j < n; j++)
                    javaList.add(0, j);
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Adding at front: ArrayList", totalTimes, initialN);
    }

    public static void addFrontLinkedList(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                LinkedList<Integer> studentList = new LinkedList<Integer>();
                s.start();
                for(int j = 0; j < n; j++)
                    studentList.insert(0, j);
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Adding at front: LinkedList", totalTimes, initialN);
    }

    public static void removeFrontArrayList(Stopwatch s, int initialN, int numTests){     
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                ArrayList<String> javaList = new ArrayList<String>();
                for(int j = 0; j < n; j++)
                    javaList.add(j + "");
                s.start();
                while(!javaList.isEmpty())
                    javaList.remove(0);
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Removing from front: ArrayList", totalTimes, initialN);
    }

    public static void removeFrontLinkedList(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;    
            for(int i = 0; i < numTests; i++){
                LinkedList<String> studentList = new LinkedList<String>();
                for(int j = 0; j < n; j++)
                    studentList.add(j + "");
                s.start();
                while( studentList.size() != 0 )
                    studentList.removeFirst();
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("removing from front: LinkedList", totalTimes, initialN);
    }

    public static void getRandomArrayList(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            int total = 0;
            Random r = new Random();
            for(int i = 0; i < numTests; i++){
                ArrayList<Integer> javaList = new ArrayList<Integer>();
                for(int j = 0; j < n; j++)
                    javaList.add(j);
                s.start();
                for(int j = 0; j < n; j++){
                    total += javaList.get( r.nextInt(javaList.size()) );
                }
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting random: ArrayList", totalTimes, initialN);
    }

    public static void getRandomLinkedList(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            int total = 0;
            Random r = new Random();
            for(int i = 0; i < numTests; i++){
                LinkedList<Integer> studentList = new LinkedList<Integer>();
                for(int j = 0; j < n; j++)
                    studentList.add(j);
                s.start();
                for(int j = 0; j < n; j++){
                    total += studentList.get( r.nextInt(studentList.size()) );
                }
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting random: LinkedList", totalTimes, initialN);
    }

    public static void getAllArrayListUsingIterator(Stopwatch s, int initialN, int numTests){

        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            int total = 0;
            for(int i = 0; i < numTests; i++){
                ArrayList<Integer> javaList = new ArrayList<Integer>();
                for(int j = 0; j < n; j++)
                    javaList.add(j);
                Iterator<Integer> it = javaList.iterator();
                s.start();
                while( it.hasNext() ){
                    total += it.next();
                }        
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting all using iterator: ArrayList", totalTimes, initialN);
    }

    public static void getAllLinkedListUsingIterator(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            int total = 0;
            for(int i = 0; i < numTests; i++){
                LinkedList<Integer> studentList = new LinkedList<Integer>();
                for(int j = 0; j < n; j++)
                    studentList.add(j);
                Iterator<Integer> it = studentList.iterator();
                s.start();
                while(it.hasNext()){
                    total += it.next();
                }
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting all using iterator: LinkedList", totalTimes, initialN);
    }

    public static void getAllArrayListUsingGetMethod(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                ArrayList<Integer> javaList = new ArrayList<Integer>();
                for(int j = 0; j < n; j++)
                    javaList.add(j);

                s.start();
                int x = 0;
                for(int j = 0; j < javaList.size(); j++){
                    x += javaList.get(j);
                }
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting all using get method: ArrayList", totalTimes, initialN);
    }

    public static void getAllLinkedListUsingGetMethod(Stopwatch s, int initialN, int numTests){
        double[] totalTimes = new double[numTests];        
        for(int t = 0; t < NUM_REPEATS_OF_TEST; t++) {
            int n = initialN;
            for(int i = 0; i < numTests; i++){
                LinkedList<Integer> studentList = new LinkedList<Integer>();
                for(int j = 0; j < n; j++)
                    studentList.add(j);

                s.start();
                int x = 0;
                for(int j = 0; j < studentList.size(); j++){
                    x += studentList.get(j);
                }
                s.stop();
                totalTimes[i] += s.time();
                n *= 2;
            }
        }
        showResults("Getting all using get method: LinkedList", totalTimes, initialN);
    }

    // for future changes
    private static interface ArrayListOp {
        public <E> void op(ArrayList<E> list, E data);
    }
    
    private ArrayListOp addAL = new ArrayListOp() {
        public <E> void op(ArrayList<E> list, E data) {
            list.add(data);
        }
    };
    
    private ArrayListOp addFrontAL = new ArrayListOp() {
        public <E> void op(ArrayList<E> list, E data) {
            list.add(0, data);
        }
    };
    
    private ArrayListOp removeFrontAL = new ArrayListOp() {
        public <E> void op(ArrayList<E> list, E data) {
            list.remove(0);
        }
    };

    private static interface LinkedListOp<E> {
        public void op(LinkedList<E> list);
    }
}